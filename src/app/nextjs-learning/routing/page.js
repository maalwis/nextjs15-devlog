...
// - **src/app folder**: In Next.js 15, the 'app' directory uses the App Router, where routes are defined by the file system. Each 'Profile.js' file maps to a route based on its path.
// - **layout.js**: A file (typically in 'app/' or subdirectories) that defines shared UI (e.g., navigation, footer) wrapping page content.
// - **RootLayout home**: Usually in 'app/layout.js', this top-level layout wraps all pages, ensuring a consistent structure across the app.
...
// Main page home for the route '/nextjs-learning'.
function Page() {
    return (
